...
// SetContext sets the context used by subsequent imports. Unset means Background.
func (n *Network) SetContext(ctx context.Context) { n.ctx = ctx }

// ClearContext returns subsequent imports to Background.
func (n *Network) ClearContext() { n.ctx = nil }
...
func (n *Network) Xhost_sock_send(fd, ptr, size int32) (status int32) {
	s, status := n.get(fd)
	if status != 0 {
		return status
	}
	...
	defer func() {
		// A byte count is the guest's only record of what left the socket, so a
		// failed close never replaces one.
		if err := finish(); err != nil && status == 0 {
			status = errnoOf(err)
		}
	}()
...
func (n *Network) Xhost_sock_recv(fd, ptr, size int32) (status int32) {
	s, status := n.get(fd)
	if status != 0 {
		return status
	}
